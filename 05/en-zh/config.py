...
def bleu(pred:torch.Tensor, y:torch.Tensor, mask:torch.Tensor, y_mask:torch.Tensor)->float:
    ...
    for i in range(len(pred)):
        pred[i] = list(spm.Decode(pred[i][0:mask[i,:].sum().item()]))
        y[i] = [list(spm.Decode(y[i][0:y_mask[i,:].sum().item()]))]
...
class Config:
    ...
    train_prefix = os.path.join(data_prefix,'train_dev.new.')
    test_prefix = os.path.join(data_prefix, 'test.clean.')
